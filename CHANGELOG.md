...
- Revert `Include file extensions in generated file imports` introduced in `v0.0.7` for TypeScript users.
...
The TypeScript team's recommendation to use `.js` extensions for `.ts` file imports causes a number of issues with the broader JavaScript ecosystem. Until this situation is rectified, ProtoScript will not emit ESM compliant extensions for TypeScript. This only impacts TypeScript users who wish to target ESM in Node.JS using the TypeScript compiler, as bundlers are not pedantic about file extensions. If you're impacted by this, please join the discussion in https://github.com/tatethurston/TwirpScript/issues/202.
